...
const Navbar = () => {
    ...
    const userItemTemplate = (item: any) => {
        return (
            <div onClick={() => {
                router.push(`/user/${item._id.$oid}`);
                setSearchedUsername(item.username);
    ...
    const end = <AutoComplete  completeMethod={searchUser} value={searchedUsername} suggestions={filteredUsers} onChange={(e) => setSearchedUsername(e.value.username ?? e.value)} itemTemplate={userItemTemplate} />
